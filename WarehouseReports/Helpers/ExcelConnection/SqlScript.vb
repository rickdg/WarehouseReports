        ...
        Public Function GetPreviewScript(loadType As LoadType, table As String) As String
            Select Case loadType
                ...
                Case LoadType.ExtraData
                    Return GetExtraPreviewScript(table)
                Case Else
...
FROM [{table}]
WHERE [Тип задачи системы] = 'Размещение' AND IIF([Складское подразделение] IS NULL, 1 = 1, [Складское место] <> [СМ-получатель]) {Placement}
...
FROM [{table}]
WHERE [Тип задачи системы] IN ('Перенос заказа на перемещение', 'Пополнение', 'Размещение') AND [Складское место] <> [СМ-получатель] {Movement}
GROUP BY [Тип задачи системы], [Складское подразделение], [Складское место], [Склад-получ#], [СМ-получатель], [Тип задачи пользователя], [Работник], [НЗ содержимого], [Номерной знак отправителя], [Загруженный НЗ]"
        ...
        Public Function GetExtraPreviewScript(table As String) As String
            Return $"SELECT	CDate(Pcs.XDate) AS Дата, Pcs.ZoneShipper AS [Складское подразделение], UnloadedLPN.QtyUnloadedLPN AS [Кол-во выгруженных НЗ], Orders.QtyOrders AS [Кол-во заказов], Pcs.AvgQtyPcs AS [Среднее кол-во единиц в строке заказа]
